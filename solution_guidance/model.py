...
from solution_guidance.logger import log_predict, log_train
...
def _model_train(df,tag,test=False):
    ...
    ## update log
    log_train(tag,(str(dates[0]),str(dates[-1])),{'rmse':eval_rmse},runtime,
...
def model_predict(country,year,month,day,all_models=None,test=False):
    ...
    ## update predict log
    log_predict(country,y_pred,y_proba,target_date,
                       runtime, MODEL_VERSION, test=test)
    
    return({'y_pred':y_pred,'y_proba':y_proba})

def nearest(target_date, available_dates):
    available_dates = pd.to_datetime(available_dates)
    target_date = pd.to_datetime(target_date)
    nearest_index = (available_dates - target_date).abs().argmin()
    nearest_date = available_dates.iloc[nearest_index].strftime('%Y-%m-%d')
    return nearest_date
